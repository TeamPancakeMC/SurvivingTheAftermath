...
public class AftermathEventUtil {
    //post start
    public static void start(BaseAftermath aftermath, List<UUID> players, ServerLevel level) {
        aftermath.setState(AftermathState.START);
        MinecraftForge.EVENT_BUS.post(new AftermathEvent.Start(aftermath,players, level));
    }
    //post ready
    public static void ready(BaseAftermath aftermath, List<UUID> players, ServerLevel level) {
        aftermath.setState(AftermathState.READY);
        MinecraftForge.EVENT_BUS.post(new AftermathEvent.Ready(aftermath,players, level));
    }
    //post ongoing
    public static void ongoing(BaseAftermath aftermath, List<UUID> players, ServerLevel level) {
    ...
    //post victory
    public static void victory(BaseAftermath aftermath, List<UUID> players, ServerLevel level) {
    ...
    //post celebrating
    public static void celebrating(BaseAftermath aftermath, List<UUID> players, ServerLevel level) {
    ...
    //post end
    public static void end(BaseAftermath aftermath, List<UUID> players, ServerLevel level) {
